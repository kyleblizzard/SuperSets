...
final class WorkoutManager {
    ...
    /// Most recently used lifts for circle buttons (max 4, newest first).
    ...
    func deleteSet(_ workoutSet: WorkoutSet) {
        ...
        // LEARNING NOTE:
        // After deleting set #2 of 4, renumber: #3→#2, #4→#3.
        // Sort by timestamp (creation order) and reassign numbers.
    ...
    /// Move a lift to the front of the recent lifts list (max 4).
    private func updateRecentLifts(with lift: LiftDefinition) {
        ...
        if recentLifts.count > 4 {
            recentLifts = Array(recentLifts.prefix(4))
        }
    }
    ...
    /// Load the 4 most recently used lifts for circle buttons.
